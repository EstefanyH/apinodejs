import dotenv from 'dotenv';
import fusion_controller from './src/interfaces/controllers/fusion_controller.js';
import storage_controller from './src/interfaces/controllers/storage_controller.js';
import history_controller from './src/interfaces/controllers/history_controller.js';

import SwapiService from './src/infrastructure/services/swapi_services.js';
import WeatherService from './src/infrastructure/services/weather_services.js';

dotenv.config();

/*
console.log("AWS_ACCESS_KEY_ID:", process.env.AWS_ACCESS_KEY_ID);
console.log("AWS_SECRET_ACCESS_KEY:", process.env.AWS_SECRET_ACCESS_KEY);
console.log("AWS_REGION:", process.env.AWS_REGION); */

const swapiService = new SwapiService();
const weatherService = new WeatherService();

const fusionController = new fusion_controller(swapiService, weatherService);
const storageController = new storage_controller();
const historyController = new history_controller();

export const fusionados = async (e) => fusionController.fusionados(e);
export const almacenar = async (e) => storageController.almacenar(e);
export const historial = async (e) => historyController.historial(e);